...
class tumblr_blog(model_oper_bz.base):
    ...
    is_submission = BooleanField(null=True)
    is_anonymous = BooleanField(null=True)
    post_author = BinaryJSONField(null=True)
    player = BinaryJSONField(null=True)
    thumbnail_width = IntegerField(null=True)
    thumbnail_height = IntegerField(null=True)
    html5_capable = TextField(null=True)
    video_type = TextField(null=True)
    permalink_url = TextField(null=True)
    thumbnail_url = TextField(null=True)
    excerpt = TextField(null=True)
    link_author = TextField(null=True)
    description = TextField(null=True)
    publisher = TextField(null=True)
    url = TextField(null=True)
    video_url = TextField(null=True)
    audio_url = TextField(null=True)
    dialogue = BinaryJSONField(null=True)
